...
import { Entity,BillboardGraphics,Label,LabelCollection,BillboardCollection,Billboard, EntityDescription,useCesium} from "resium";
import {
  Transforms,
  Cartesian3,
Color,
...
const Marker = ({lon,lat,color,idd,popupContent}) => {
  ...
  const PopupObj = useRef();
  const popupID="popup"+idd;
  const closePopup = () => {
    PopupObj.current?.switchElementShow(false);
...
const showPopup=()=>{
  ...
  if (element && popupContent) {
    PopupObj.current = new Popup(viewerCs.viewer, {
      position: [lon, lat],
      element,
      offset:[0,-30]
    });  
